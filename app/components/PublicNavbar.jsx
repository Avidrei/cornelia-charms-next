...
export default function PublicNavbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 bg-white transition-shadow duration-300 ${isOpen ? "" : "shadow-md"}`}>
      <div className="flex items-center justify-between px-6 py-4 sm:py-6 md:py-8 lg:py-9 xl:py-9 md:px-20">
        ...
        <Link href="/">
          <Image
            src={logo}
            alt="Cornelia Logo"
            className="w-auto h-8 pl-2 transition-all sm:h-9 md:h-11 lg:h-13 xl:h-14 hover:brightness-75 hover:saturate-130"
          />
        </Link>

        {/* Desktop Nav */}
        <div className="hidden space-x-20 text-3xl tracking-wide uppercase xl:text-4xl lg:flex text-cornelia-darkpink bebas">
          <Link href="/about" className={`transition-colors duration-200 hover:text-pink-800 ${pathname === "/about" ? "text-pink-800" : ""}`}>About Us</Link>
